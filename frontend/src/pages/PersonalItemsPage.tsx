...
export function PersonalItemsPage({ session }: { session: Session }) {
  ...
  const patchPersonal = usePatchItem(session, (item) => {
    setSelectedId(item.id);
    invalidatePersonal();
  });

  const archivePersonal = useMutation({
  ...
  const restorePersonal = useMutation({
  ...
  const savePersonal = useSaveItemWithReminder(session, (item) => {
    setSelectedId(item.id);
    invalidatePersonal();
  });

  const undo = useUndo();
  const toggle = useToggleDone(session, 'personal');
  ...
  function archiveWithUndo(item: Item) {
    archivePersonal.mutate(item, {
      onSuccess: () => undo.pushUndo(`已归档「${item.title}」`, () => restorePersonal.mutate(item)),
    });
  }
    ...
    patchPersonal.mutate(
      { item, payload: { status: 'done' } },
      {
        onSuccess: (updated) => {
          ...
          invalidatePersonal();
          undo.pushUndo(`已跳过「${item.title}」`, () => patchPersonal.mutate({ item: updated, payload: { status: 'planned' } }));
        },
      },
    );
  }
  ...
  return (
    <section className="page personal-page">
      ...
      <div className="list-controls">
        ...
        {view === 'current' && (
          <button
            className={filterOpen || personalFilterCount > 0 ? 'chip active' : 'chip'}
            type="button"
            onClick={() => setFilterOpen((open) => !open)}
          >
            筛选{personalFilterCount > 0 ? ` ${personalFilterCount}` : ''}
          </button>
        )}
        <div className="toolbar-right">
          <input placeholder="搜索标题、备注或标签" value={search} onChange={(event) => setSearch(event.target.value)} />
      ...
      {view === 'current' && filterOpen && (
        <div className="calendar-filter-panel">
          <QuickFilterBar value={quickFilter} options={personalQuickFilters} onChange={setQuickFilter} />
          <button className={highPriority ? 'chip active' : 'chip'} type="button" onClick={() => setHighPriority(!highPriority)}>
            高优
          </button>
          {personalFilterCount > 0 && (
            <button
              className="ghost sm"
              type="button"
              onClick={() => {
          ...
          <SaveViewButton session={session} spec={{ page: 'personal', quickFilter, highPriority, search }} />
        </div>
      )}
              ...
              <section className="personal-group" key={group.label}>
                ...
                {group.items.map((item) => (
                  <PersonalItemRow
                    ...
                    onRestore={() => restorePersonal.mutate(item)}
                  />
                ))}
              </section>
            ))}
      ...
      {selected && (
        <PersonalItemDrawer
          item={selected}
          session={session}
          saving={savePersonal.isPending || patchPersonal.isPending}
          error={savePersonal.error?.message ?? patchPersonal.error?.message}
          onClose={() => setSelectedId(null)}
          onSave={(draft, reminderTouched, tagIds) => savePersonal.mutate({ item: selected, draft, reminderTouched, tagIds })}
          onStatus={(status) => patchPersonal.mutate({ item: selected, payload: { status } })}
          onReschedule={(payload) => patchPersonal.mutate({ item: selected, payload })}
